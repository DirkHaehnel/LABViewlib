        ...
        switch (LOWORD(wParam))
          {
           case IDOK:
...

            {
                ...
                if (ulSyncSource & OLC_FG_CSYNC_SPECIFIC_SRC)
                  {
                     ulSyncSource = MAKELONG(OLC_FG_CSYNC_SPECIFIC_SRC, usNewSource);   
                      ...
                      if ( ! OlImgIsOkay(Status) )
                         PrintStatus(hDlg, Status, "Unable to set sync source.", "Sync Source Error");
                       }
