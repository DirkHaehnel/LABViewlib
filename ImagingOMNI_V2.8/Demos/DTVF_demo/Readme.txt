...
that you use the DT Vision Foundry Export tool. For more 
information, refer to the DT Vision Foundry documentation. 

3. GENERAL: If a tool has AUTOSTART enabled, the tool may remain
in the Windows task after you close the DT Vision Foundry 
application. If this happens, close the tool manually. 

4. GENERAL: Under certain circumstances, the file ILERR.OUT may 
appear on your desktop. If this happens, you can just delete this 
files from the desktop. 

5. POINT & CLICK SCRIPT TOOL: Point & click scripts support 
relative paths in DT Vision Foundry Ver. 3.5 and higher. If you 
have a point & click script created in a previous version of 
DT Vision Foundry, run the point & click script from the 
location where it was created, then save the script. The script 
will now support relative paths. 

6. POINT & CLICK SCRIPT TOOL: For some tools (such as the Image 
Classifier, Gauge, and Prompt tools), the tool's point & click 
script variables are not immediately brought into the Point & Click 
Script tool. If you have enabled the point & click script variables, 
but the variables do not appear in the Point & Click Script tool, 
click the Step button until you have stepped past the appropriate 
tools's script item. The variables should then appear. 

7. DIGITAL I/O TOOL: The Digital I/O tool provided with DT Vision 
Foundry Ver. 3.5 is not compatible with the DT2820 (Digital I/O) 
tool provided in previous versions of DT Vision Foundry. If you 
created a point & click script using the DT2820 Digital I/O tool, 
you must edit your script to include the new Digital I/O tool. 
...
19. STATUS TOOL: DT Vision Foundry expects only one Status tool 
to be open at a time. Therefore, do not use the Edit\Copy and 
Edit\Paste options in the Point & Click Script tool to create 
more than one instance of the Status tool in your point & click 
script. If you do, unexpected results may occur. 

20. THRESHOLD TOOL: If you want to provide your own name for the 
...
DT Vision Foundry is shipped with several examples to help you get 
started creating either point & click scripts or your own custom 
tools and application programs. 

-- The C:\PROGRAM FILES\DATA TRANSLATION\DT VISION FOUNDRY\SCRIPTS 
   directory contains several point & click scripts. These are 
   intended for DT Vision Foundry users who want to use the 
   Point & Click Script tool to create simple scripts to automate 
   repetitive tasks within the DT Vision Foundry environment.
   Programming experience is helpful, but not required. 

-- The C:\PROGRAM FILES\DATA TRANSLATION\DT VISION FOUNDRY\
   ...
   DT Vision Foundry API within the Visual C++ 6.0 environment. 
...
system. If necessary, you can use the debug version of DT Vision 
Foundry (DTVFD.EXE) to debug your custom tools. DTVFD.EXE is 
located in C:\PROGRAM FILES\DATA TRANSLATION\DT VISION FOUNDRY\
BIN, by default. 

2. Before you compile a custom tool (project) that you created 
using the DT Vision Foundry APIs, make sure that you add the files 
DT_TOOL.CPP and DT_TOOL.H to the project. DT_TOOL.CPP and DT_TOOL.H
are located in C:\PROGRAM FILES\DATA TRANSLATION\DT VISION FOUNDRY\
C++ DEVEL\EXAMPLES\TOOLS\CHANGE, by default. 

3. To run a custom application that you created using the DT 
Vision Foundry APIs, the files DTAPI.DLL, DTDEVMAN.DLL, and
DTTOOLS.DLL must be located in the same directory as your custom 
application. These files are located in C:\PROGRAM FILES\DATA 
TRANSLATION\DT VISION FOUNDRY, by default. If your custom 
application is located in a different directory, copy these files 
and place it in the directory containing the custom application. 
