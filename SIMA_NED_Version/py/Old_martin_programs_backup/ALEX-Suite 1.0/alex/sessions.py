...
from .models import ReducParams, Thresholds, EvsS, Metadata, FretCorrections, PlotOptions
...
#                     raise IOError('The specified archive contains files I do not recognise.')
    ...
    def create_pdf(self, path):
        ...
        for title, obj in [('<b>Thresholds</b>', self.thresholds), ('<b>Accurate FRET</b>', self.corr), ('<b>Metadata</b>', self.metadata), ('<b>Burst Search Parameters</b>', self.params), ('<b>Plot Options</b>', self.plot_options), ]:
        ...
        f = Frame(h_offset + I.drawWidth, page_height - I.drawHeight - v_offset, (1./4)*(page_width - 2*h_offset), I.drawHeight, showBoundary=0)
        f.addFromList(elements[2:6], c)

        # Bottom-left column
        f = Frame(h_offset, v_offset, .5*page_width - h_offset, page_height - I.drawHeight - 2*v_offset, showBoundary=0)
        f.addFromList(elements[:2] + elements[6:8], c)

        # Bottom-right column
        f = Frame(.5*page_width, v_offset, .5*page_width - h_offset, page_height - I.drawHeight - 2*v_offset, showBoundary=0)
