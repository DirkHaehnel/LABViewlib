...
import fit
from models import EvsS, TitrationContainer
import string
...
class AbstractFunction(HasTraits):
    ...
    def __unicode__(self):
        ...
        args = string.join(values, ', ')  # Join them together with a comma to form the function argument.
...
class Beta(AbstractFunction):
    ...
    # @staticmethod
    def func(self, x, p):
        self.A, self.a, self.b = p
        # x0, sig, A = p
        return self.A*beta.pdf(x, self.a, self.b)
    ...
    def get_bounds(self):
        ...
        bounds.append((self.a, np.nextafter(self.a, 1), ) if self.a_fixed else (None, None, ))
        bounds.append((self.b, np.nextafter(self.b, 1), ) if self.b_fixed else (None, None, ))
        print bounds
        return bounds
...
class TitrationCurve(AbstractFunction):
    k = Float(1.0)
    k_fixed = Bool(False)
    b = Float(1.0)
    b_fixed = Bool(False)
    ...
    def get_bounds(self):
        ...
        bounds = []
        bounds.append((self.k, self.k + float('1E-20'), ) if self.k_fixed else (None, None, ))
        bounds.append((self.b, self.b + float('1E-20'), ) if self.b_fixed else (None, None, ))
        return bounds
...
class Guess1D(HasTraits):
    ...
    @on_trait_change('function_type.updated') #function_type.+,
    def update_name(self):
        print 'updating name!'
...
class GuessesToFit1D(HasTraits):
    ...
    #     print 'run fitting'
    def __init__(self, *args, **kwargs):
        super(GuessesToFit1D, self).__init__(*args, **kwargs)
        self.guesses = []
...
class GuessesToFitTitration(HasTraits):
    fittings = List(GuessesToFit1D)
    data_container = Instance(TitrationContainer)
    run_fitting = Event
    titration_fit = Instance(fit.Fit)

    def __init__(self, *args, **kwargs):
        super(GuessesToFitTitration, self).__init__(*args, **kwargs)
        self.fittings = []

    def execute(self):
        if len(self.fittings) != len(self.data_container.fret_containers):
            pass
        elif len(self.fittings) < 3:
            pass
        else:
            # First fit to the two extremes.
            self._fit_fret_container(0)
            self._fit_fret_container(-1)
            # Than to the rest
            for key in range(1, len(self.data_container.fret_containers) - 1):
                self._fit_fret_container(key)

            # Calculate the titration curve
            ratio = [0.0]
            for fitting in self.fittings[1:-1]:
                Area1 = fitting.guesses[0].get_area()
                Area2 = fitting.guesses[1].get_area()
                ratio.append(Area2 / (Area2 + Area1))
            ratio.append(1.0)
            self.data_container.ratio = np.array(ratio)

            print ratio, self.data_container.concentrations
            # convert from nanomolar to molar.
            concentrations = np.array(self.data_container.concentrations) * 10**-9
            # Fit the ratios to the concentrations using a titration curve
            self.titration_fit = fit.Fit(TitrationCurve(), concentrations, ratio, p0=[1.0, 1.0])
            self.titration_fit.execute()

    def _fit_fret_container(self, key):
        """
        Fit the fret_container corresponding to key.
        :param key:
        :return:
        """
        self.fittings[key].execute(
            x=self.data_container.fret_containers[key].Ecentres,
            y=self.data_container.fret_containers[key].H_Epr
        )
...
func_dict = {'Gaussian': Gaussian, 'Zero': Zero, 'Constant': Constant, 'Exponent': Exponent,
             'NegativeExponent': NegativeExponent, 'Linear': Linear, 'Beta': Beta}