from traits.api import HasTraits, Instance, ListUnicode, Array, Str, on_trait_change, Int, Event, List, Unicode
import subprocess
import csv
import os
import numpy as np
...
class LazyFile(HasTraits):
    ...
    The idea behind this is that we are not always interested in every datafile all the time.
    So the program knows which datafiles and types are available for every experiment, 
    but only reads them into memory when needed.
    """
    file_name = Str
...
class SessionFile(CacheFile):
    ...
    def read(self):
        ...
        #     titles = ['BURST SEARCH PARAMETERS', 'THRESHOLDS', 'EXPERIMENTAL', 'ACCURATE FRET', 'PLOT OPTIONS',
        #               'BIN CENTERS', '2D HISTOGRAM', 'DATA']
        #     sections = [[] for i in range(len(
        #         titles))]  # Create an empty list of lists to hold all data after extraction from file. +1 because there is some bull before the valuable info starts.
        #     key = 0
        ...
        raise NotImplementedError('The write was changed to allow for diverent E and S ranges. Adapt read to this in the future.')
    ...
    def write(self):
        ...
        with open('{}{}'.format(root, ext), 'w') as f:
            ...
            csv_writer.writerow(['BIN CENTERS E', 'E HISTOGRAM',])
            for x, E in zip(self.evss.Ecentres, self.evss.H_Epr):
                csv_writer.writerow([x, E])

            csv_writer.writerow(['BIN CENTERS S', 'S HISTOGRAM'])
            for y, S in zip(self.evss.Scentres, self.evss.H_Sraw):
                csv_writer.writerow([y, S])
...
class BurstFile(LazyFile, CacheFile):
    ...
    def _get_header_size(self):
        ...
        """
        return 3 + len(self.setting.get_names())

    def _read_to_np(self):
        """ Opens a file and turns the result into a structured array """
        return np.genfromtxt(self.file_name, delimiter=",", skip_header=self._get_header_size(),
                             dtype=self._get_dtype(), invalid_raise=False, comments=None)
...
class Dataset(HasTraits):
    ...
    def get_photons(self):
        with self.photon_file as photons:
            return photons

    @staticmethod
    def run_burst_search_cpp(file_name, settings):
        ...
        subprocess_path = os.path.join(os.path.dirname(os.path.realpath(__file__)),
                                       "alex_tools.exe" if _platform == 'win32' else "alex_tools")

        params_list = ['{}'.format(value) for value in settings.get_values()]
        try:
            print subprocess.check_call([subprocess_path, file_name] + params_list)
    ...
    @classmethod
    def _file_exists(cls, wrapped_file_name, settings):
        ...
        try:
            with open(wrapped_file_name, 'rU') as file_handle:
                ...
                for key, row in enumerate(raw_data):
                    ...
                    raise EOFError(
                        'End of file reached without finding \'DATA\'. This indicates an impropper wrapped burst file.')
                # Naively assumes the chance of two set's checking out the same while they are not is so slim, this assumption is save.
                return set(cls._str_list_to_types(metadata)) == set(cls._str_list_to_types(settings.get_values()))
        except IOError:
            return False
...
class DataManager(HasTraits):
    ...
    sm_path = Str
    # List containing all sm files in the directory
    files = ListUnicode
    # List containing all sm files to be converted to wrapped burst files
    selected_files = ListUnicode
    ...
    def execute(self):
        ...
        # Nothing special, most of the times users will be opening .sm files.
            ...
            self.datasets = []
            self.number_of_dataset = len(self.selected_files)
            data_to_merge = []
            for key, file_name in enumerate(self.selected_files):
    ...
    def set_files(self):
        ...
        self.files = [os.path.join(self.sm_path, item) for item in os.listdir(self.sm_path) if
                      self.valid_file(item)] if self.sm_path else []
    ...
    def _file_types(self):
        ...
        """
        parts = set([item for file_name in self.selected_files for item in file_name.split('.')])
