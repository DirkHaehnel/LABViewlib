...
from traits.api import HasTraits, Instance, ReadOnly, Event, Enum, on_trait_change, undefined, List, Int
from models import EvsS, BVA, Dataset, PlotFormatOptions, EvsSLayers, DataContainer, TitrationContainer, BurstPropertiesOptions, BurstProperties
from functions import GuessesToFit, GuessesToFit1D, GuessesToFitTitration, TitrationCurve
...
class TitrationPlot(MPLBasePlot):
    """ View for a titration experiment.
    The view is two fold: before fitting, we can only look at the stack plot.
    After fitting, it is possible to look at the titration curve as well.
    We'll have to figure out how to incorporate this.
    """
    mode = Int(0) # 0: 'stack_plot', 1: 'titration_curve'
    data_container = Instance(TitrationContainer)
    axes = List(matplotlib.axes.Axes)
    fitting = Instance(GuessesToFitTitration)

    def __init__(self, *args, **kwargs):
        super(TitrationPlot, self).__init__(*args, **kwargs)
        self.update()

    @on_trait_change('data_container:updated, mode')
    def update(self):
        """ Update the plot to reflect changes to data_container """
        if self.fig:
            # self.fig = plt.figure(figsize=(self.format_options.width, self.format_options.height),
                              # dpi=self.format_options.dpi)
            self.fig.clear()
            ...
            self.plot()
        super(TitrationPlot, self).update()

    def plot(self):
        if self.mode == 0:
            self.plot_stack_plot()
        else:
            self.plot_titration_curve()

    def plot_titration_curve(self):
        """
        Fitting must be completed before this makes sense.
        """
        # Plot the fit
        func = TitrationCurve()
        x = np.linspace(0, np.max(self.data_container.concentrations) * 10**-9)
        ax = self.fig.add_subplot(111)
        ax.plot(x, func.func(x, self.fitting.titration_fit.popt))
        # Plot the points
        concentrations = np.array(self.data_container.concentrations) * 10**-9
        ax.scatter(concentrations, self.data_container.ratio)


    def plot_stack_plot(self):
        """ Plot's all relevant items """
        self.axes = []
        # The first one has to be done manually because it's the offset position
        self.plot_first(self.data_container.fret_containers[-1])
        for fret_container in self.data_container.fret_containers[::-1][1:]:
            self.axes.append(self._plot_hist(fret_container))

        # Add fitting if available
        self.plot_fitting()
        self.plot_guesses()
        # Make it nice!
        self.format_plot()
    ...
    def plot_fitting(self):
        try:
            # Same length indicates both are initiated.
            if len(self.data_container.fret_containers) == len(self.fitting.fittings):
                for key, fret_container in enumerate(self.data_container.fret_containers):
                    # Take the fitting instance and axes corrosponding to this key.
                    y = self.fitting.fittings[key].fit_instance.func(
                        fret_container.Ecentres,
                        self.fitting.fittings[key].fit_instance.popt
                    )
                    self.axes[::-1][key].plot(fret_container.Ecentres, y, color='black', linewidth=2)
        except AttributeError:
            print 'Plotting fits failed due to AttributeError.'
    ...
    def plot_guesses(self):
        ...
        key = 0
        for fitting, fret_container in zip(self.fitting.fittings, self.data_container.fret_containers):
            for guess in fitting.guesses:
                E = guess.function_type.func(fret_container.Ecentres, guess.function_type.get_params())
                self.axes[::-1][key].plot(fret_container.Ecentres, E, color='black', linestyle='dashed', linewidth=2)
            key += 1

    def _plot_hist(self, fret_container):
        """
        Plot a 1D fret histogram and return the 1D axis.
        :return: matplotlib.axes.Axes
        """
        ax = self.divider.append_axes(
            "top",
            size='100%',
            sharex=self.axes[0],
        )
        ax.bar(
            fret_container.Eedges[1:],
            fret_container.H_Epr,
            width=fret_container.Eedges[0] - fret_container.Eedges[1],
            color='grey'
        )
        return ax

    def plot_first(self, fret_container):
        """
        :return:The first condition to be plotted will be used as the offset to
        all other plots.
        """
        self.axes.append(self.fig.add_subplot(111))
        self.mappable = self.axes[0].bar(
            fret_container.Eedges[1:],
            fret_container.H_Epr,
            width=fret_container.Eedges[0] - fret_container.Eedges[1],
            color='grey'
    ...
    def format_plot(self):
        ...
        Make it pretty!
        """
        # Disable x labels for all but the bottom one
        for ax in self.axes[1:]:
            plt.setp(ax.get_xticklabels(), visible=False)
        # Set number of y ticks to 3, x ticks to 5.
        for ax in self.axes:
...
class EvsSPlot(MPLBasePlot):
    ...
    def plot_S_range(self):
        ...
        x = np.linspace(self.data_container.plot_options.E_min, self.data_container.plot_options.E_max, 1000)
    ...
    def plot_E_range(self):
        """ Show the S range depicted in the E histogram by drawing lines across the EvsS histogram. """
        y = np.linspace(self.data_container.plot_options.S_min, self.data_container.plot_options.S_max, 1000)
    ...
    def set_show_S_range(self):
        ...
        """
        self.evss_layers.show_S_range = not (self.data_container.thresholds.S_max_1D == self.data_container.plot_options.S_max and self.data_container.thresholds.S_min_1D == self.data_container.plot_options.S_min)
    ...
    def set_show_E_range(self):
        ...
        Do not show it if the range is the default range, because it doesn't look nice.
        """
        self.evss_layers.show_E_range = not (self.data_container.thresholds.E_max_1D == self.data_container.plot_options.E_max and self.data_container.thresholds.E_min_1D == self.data_container.plot_options.E_min)


class BurstPropertiesPlot(MPLBasePlot):
    data_container = Instance(BurstProperties)
    burst_properties_options = Instance(BurstPropertiesOptions)

    def __init__(self, *args, **kwargs):
        """ Link this view to an EvsS object so we can view it."""
        super(BurstPropertiesPlot, self).__init__(*args, **kwargs)
        self.update()
    ...
    def update(self):
        ...
        if self.fig:
            ...
            self.plot()
        super(BurstPropertiesPlot, self).update()

    def plot(self):
        self.ax = self.fig.add_subplot(111)
        # self.ax.hist(self.data_container.data['Tau'], bins=100)
        self.ax.bar(
            self.data_container.bin_edges[:-1],
            self.data_container.hist,
            width=self.data_container.bin_edges[1] - self.data_container.bin_edges[0],
            color='grey'
        )
        self.format_plot()

    def format_plot(self):
        mode = self.burst_properties_options.mode
        column = self.burst_properties_options.column
        self.ax.set_ylabel(
            '# of Bursts',
        ...
        self.ax.set_xlabel(
            '{} /Hz'.format(column) if mode == 2 else column,
            horizontalalignment='center',
            verticalalignment='bottom',
        )
